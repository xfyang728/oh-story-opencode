...
> 运行环境兼容性：Claude Code / OpenCode / Codex / ZCode / OpenClaw 是内置适配目标；NarraFork、Web AI、自定义 Agent 等能读取项目文件的环境，可按本 skill 执行长篇流程。检查专业 agent 时按 `.opencode/agents/{agent}.md` → `.claude/agents/{agent}.md` → `.codex/agents/{agent}.toml` 查找，OpenCode 环境用 Task 工具以 `subagent_type` spawn；找不到、Codex 返回 `unknown agent_type`，或检测到 `.zcode/`（ZCode 3.3.4 不执行项目 custom agents）时，直接 solo/direct 执行并报告 fallback。
>
...
| 设定/文风.md（自定义文风·优先级最高） | 本书 | 用户自写（主会话可代写）；导入/拆解不覆盖 | Phase 4 每章写作前：含实质内容则取代对标文风作权威风格基 |
