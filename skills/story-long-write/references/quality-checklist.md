...
- [ ] 配角是否有存在感？
- [ ] 反派逼格是否匹配当前阶段？
- [ ] 决策权移交：黄金三章后 ≤5 章内主角完成首次主动决策（非执行他人方案）？
- [ ] 反派在场度：明面反派缺席 >8 章须安排代理压力源（代理人出手/倒计时推进/第三方实锤转述）？

### 文风对齐检查（有 `文风.md` 时必查；v2.1 新增）

- [ ] `node scripts/style-metrics.js --profile {文风路径} 正文/第XXX章_*.md` 无 blocking 偏离？
- [ ] **作者指纹词全部在带内**（不是"零出现"）？被压掉 = 去 AI 味把作者声纹清了，优先级最高。
- [ ] 负空间没被违反？（叙述者不升华、主角不自己拆封悬念、同段不双给悬念与答案、人物不当科普嘴）
- [ ] 排版契约落实？（缩进形态 / 段间空行 / 引号体系与 `文风.md` 声明一致，且全书不混用）
- [ ] 揭示/打脸章：延迟揭示三段式写足？**反应链篇幅 ≥ 主角段落 ×3**？
- [ ] 本章是否有"连续三个以上总括式短句而不见具体动作/实物/对话"的概叙段？跳层是否超过一次？
