...
> ⚠️ **本节仅适用于长篇写作。** 与角色状态快照同文件维护（`追踪/角色状态.md`），Phase 4 每章写完后随角色状态一并更新。

对每一任明面反派，在其条目下追加以下字段块：
...
- **反派在场度**：
  ...
  - 最后正面出场章：第{N}章
  - 已缺席章数：{当前章号 - N}
  - 当前代理压力源：{代理人出手 / 倒计时推进 / 第三方实锤转述；无则填「无」}
  - 阈值告警：已缺席 >8 章且代理压力源为「无」→ 压迫感断供，须安排代理压力源或让反派重新正面登场
```

### 更新规则

1. 明面反派正面出场（当面交锋/直接施压）时，重置「最后正面出场章」与「已缺席章数」
2. 反派未出场的章节，「已缺席章数」每章 +1；若本章有代理压力源，同步更新该字段
3. 「已缺席章数」>8 且代理压力源为「无」时触发阈值告警——禁止压迫感长期仅靠转述维持
