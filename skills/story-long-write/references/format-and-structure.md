...
> **冲突裁决（先读）**：缩进形态、段间空行、引号体系三项，**只要项目文风文件（`设定/文风.md` 或对标
> `文风.md` 的「缩进与段间」「引号体系」字段）有明确声明，就以文风声明为准**；无文风文件或文风未声明时，
> 才走本节的平台默认。理由：这三项是**排版契约**（读者看到的样子），不是风格偏好；实测已有对标书
> 用 2 个全角空格缩进、弯引号 `“”`，被平台默认刷成无缩进＋角引号后三章之内出现两套体系。
> 具体判据与校验脚本见 `workflow-chapter.md` 步骤 8b 与 `scripts/style-metrics.js`。
...
- 正文相邻段落之间**只允许一个换行符 `\n`**；不得出现空行或连续换行 `\n\n`（紧密排列）。
- 无缩进（平台渲染器自行处理，不需要 `　　` 或空格）。**但对标书正文若统一用 2 个全角空格 `　　` 缩进，以对标书为准**（见本节顶部冲突裁决）——缩进形态属于排版契约，照抄对标书。
...
对话与引用**默认**使用半角双引号 `""`（文风文件声明了其他体系时从其声明，见下方「默认用」条）：

| 优先级 | 格式 | 适用平台 |
|--------|------|----------|
| 首选 | `"说话内容"` | 短篇默认、番茄 |

**默认用 `""`**：对话与引用使用半角双引号，任何平台（含知乎盐言）不得改用 `「」`/`『』`。
**但当文风文件声明了引号体系时从其声明**（如对标书用弯引号 `“”`，就用 `“”`；声明角引号就用角引号）。
**禁止混用**：同一本书内不得一半角引号一半弯引号；`normalize-punctuation.js --quote-mode` 必须与声明一致。
...
**通用原则**：小节标记默认 `###1.`（短篇通用格式）；对话引号**默认** `""`，不使用 `「」`——但当项目文风文件声明了引号体系时从其声明（长篇小说尤其常见：对标书用弯引号 `“”` 就照用），且同一本书不得混用。
