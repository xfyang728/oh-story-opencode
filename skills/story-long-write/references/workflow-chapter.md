...
3. **写前准备**（下面的 3 步是核心方法在单章写作中的落地：筛选状态 → 召回模块 → 确认意图）：
   ...
   - **文风三表必读**：读到文风文件（自定义或对标）时，除句长/标点/锚点外**必须**取出并注入写作 prompt 的还有三张表——「作者指纹词（正向白名单）」「负空间（作者不写什么）」「硬禁高危组合」。缺这三张表说明是旧版文风文件：提示按 `style-profile-generator.md` Step 4b 重建，本次按"无基线"处理（**不阻塞**），并在意图确认里注明。
...
8b. **文风偏离自检**（"必须像谁"的门；与步骤 8 的字数门同等地位，不是可选项）：
   - 跑 `node scripts/style-metrics.js --profile {文风路径} 正文/第XXX章_*.md`（脚本在 `story-long-write/scripts/`）。
   - **判定按 tier 分级**（v4，依据 8 章实测）：
     · **`hard` 类 → blocking**：最长段上限、段落绝对硬线（≥80 字／≥60 字且 ≥5 拍）、作者指纹词被清零、引号混用、缩进缺失、组合级 AI 句式。**这些是作者本人也不会违反的**。
     · **`voice` 类 → 只作 advisory**：对话占比、叹号、问号、省略号、破折号、场次切分符、面板密度、段长分布。判定用 **P10-P90 分位带**（文风未给分位时回退 `0.5×~1.6×`）。
     · **`soft` 类 → advisory**：概叙比等。
   - **为什么声纹类不能 blocking**：作者逐章方差极大（实测变异系数：叹号 **68%**、破折号 **95%**、对话占比 28%）。用「均值 ±15%」实测只能覆盖作者自己 **2/8**（叹号）、**1/8**（破折号）的章节——门曾对**作者刚写的三章**报 `blocking 2`（叹号判"偏低 −45%"、破折号判"偏高 +264%"）。**门要发现的是"作者不会那么写"，不是"作者不常那么写"**。
   - **`hard` 项命中 → 回步骤 6 重写对应段落**，不是调指标、也不是删段落：
     · `超长段 / 多拍长段` → **按句号拆段**（拆段，不是删句）
     · `作者指纹词被压掉` → 按 `banned-words.md`「判定优先级」第 1 条把词加回，**只改词、不改事**（最高优先级）
     · `引号体系不一致 / 混用` → 按 `文风.md` 声明的体系统一，并让步骤 12 用对应 `--quote-mode`
     · `缩进缺失` → 按文风声明的缩进形态（本书 = 2 个全角空格）
   - **`voice` 项命中（只提示）→ 逐条读原文判断**，不机械回改：
     · 若偏离方向与"这一段想交付的情绪"一致（放榜/狂欢章叹号上浮、冲关章破折号上浮），**保留**；
     · 若是无意识的疏懒（通篇平均、无峰谷），按提示方向调。
     · 判据是"这一章读起来像不像作者"，不是"数值是否落在带内"。
   - **场景切分符形态**：一律用三点 `...`（独立成行），**不用中文 `……`**——`……` 会被同时计入 `scene_cuts` 与 `ellipsis_per_kilo`（双计），迫使你在"删切分符"与"省略号超标"之间二选一。见 `文风.md`「场景切分符形态」。
   - **无法自检时的降级**：`文风.md` 写 `metrics: unavailable` 时跳过本步骤（不阻塞），但必须在意图确认里注明"本章无指标基线，仅按锚点语感对齐"。
...
10b. **作者基线比对**（与禁用词扫描互为反面，缺一不可）：
   - 读 `文风.md` 的「作者指纹词（正向白名单）」表。**凡白名单词在步骤 11 被列为"可替换"的，一律保留**——这些词的缺失本身就是漂移，比多一个词更严重。判据见 `banned-words.md`「判定优先级」。
   - 读「负空间（作者不写什么）」。负空间违反优先于禁用词命中处理：宁可留一个一级词，也不要把叙述者解释、主角自我宣言、同段双给悬念与答案写进来。
   - 读「硬禁高危组合」。这几条**即使作者原文出现过同类词也不得复现组合形态**。
   - 三张表都缺失时（旧版文风文件）：提示按 `style-profile-generator.md` Step 4b 重建文风，本次按"无基线"处理并在意图确认里注明。
10c. **新角色重名检查**（v3 新增，必查）：本章若首次引入**具名**角色或势力，落笔前必须确认不与对标书既有角色重名：
   - 查 `{对标书路径}/角色/*.md` 与（若原文可得）`grep` 原著全文的该名字，命中即**换名**并回写细纲，不得沿用。
   - 重名不是小事：实测踩过——续写把「沈青」当作小世界丹师/丹铺东家，而该名在原著出现 **167 次**，
     是**精英班班主任**（第460章港口送行时她本人就在场）。作者真实续写里她仍是老师，新出场的女主管叫「阮玉」。
   - 四道门都拦不住重名（脚本不查名），所以必须人工/检索确认。一次性路人可只做本项检查、不建角色档。
11. **禁用词扫描**：先过**最毒句式速查**（实测最易漏，命中即改）：①「不是A，(而)是B」全家族——含「没有X，没有Y(，只是Z)」排比否定、「是B，不是A」反序、「他没X，也没有Y。他只是Z」先抑后扬，；②声线反差「声音不大/不高…却…」；③「，带着……」万能状语；④预告/总结收尾「没人知道…」「(这)才刚刚开始/开头」「正朝着…压过去」「即将拉开序幕」「这一刻…」；⑤叙述里短词加引号强调（他是被请来"把关"的）。再复核 detector 的 `formulaic-parallelism` advisory：跨段「不是A。/也不是B。/只是C。」、`至于X不X，怎么X`、同动词 `不V A，不V B` 即使写在台词里也不能跳过，确属人物当场的功能性表达才保留。然后对照 `references/banned-words.md` 全表：**先按「判定优先级」判级——组合级不可豁免；单字级须先查 `文风.md` 的作者指纹词白名单（命中且频次在带内 → 豁免，照常写）**；一级词带外命中即替换；二级词（低频/语境相关）高频出现时替换，偶发可参考 `references/anti-ai-writing.md` 定性裁定。
11b. **结构件点检**（五个可证伪项；不达标 → 回步骤 6 重写，不许靠补旁白糊过去）：
   - **延迟揭示是否成立**：本章若定位为"揭示/放榜/翻盘"，检查是否写足了「假死（把最坏结论做实）→ 反向假信号（让内外都以为还有转机）→ 最重落点（答案压在最后）」三段。**若主角在本章就自己开口把悬念说破，判 blocking**——悬念必须由外部期待与落差承担。
   - **反应链是否走完**：装逼/打脸/揭露章，在场配角的反应是否按「辱骂→见牌/见数呆滞→失态→退场或服软」写足，且**反应段落篇幅 ≥ 主角段落 ×3**？只写主角动作或一句"脸色铁青"= 不达标。
   - **概叙比是否超标**：本章是否用旁白概括替代了连续镜头（如"第三层。第五层。如履平地。"式跳层）？关键情节必须拍成场景；跳层只能用于非关键过渡，且整章跳层不超过一次。
   - **细节推进器核对**（v5 新增，最高频的失手项）：本章所有**设定／背景／数字**信息，逐条过 writing-craft.md「细节是推进器」的四问——
     ① 推出什么可证伪的结论？② 改变了谁的决定或预期？③ 数字能否用于判断（标尺 vs 背景）？④ 删掉情节是否照走？
     **四条里三条不合格 → 判不达标**。注意：**判据是信息的功能，不是说话人身份**——"让利益方说出来"只是形式，把背景塞进反派嘴里照样是上课。
   - **章首/转场细节是否可换算**：每章第一个细节（尤其章首与每个转场后的第一句）必须是威胁评估／资源盘点／判断依据，**不能是氛围描写**。自检：删掉这句，读者对当前处境的理解会不会变差？不会变差 → 它是抒情占位，回写。
...
检查四个维度：(1) **情绪交付**——每章是否交付了细纲中规划的目标情绪？(2) **契约风险**——按 `references/reader-contract-and-progression.md` 检查因果权 + 结算权、关键节点四问、期待所有权、期待债、终局储备（透支两问）与换书债；章级推进按权威文件的七类状态分档（快节奏保留可见事件/爽点下限），强弱相对本书题材与对标判断，标记 契约安全 / 需补强 / 契约破坏；契约破坏 先修正文或修后续纲。(3) **技术质量**——一致性、格式、禁用词、排版契约（缩进/段间/引号体系与 `文风.md` 一致）。(4) **文风对齐**——步骤 8b 的指标门全绿、步骤 10b 的白名单词没被清掉、负空间没被违反。参考 [references/quality-checklist.md](quality-checklist.md) 中的通用检查和长篇专项清单。

**正文元信息扫描**：按上方步骤 10 清掉标题行以外的写作工程词，再进入其他检查。`check-degeneration.js` 会确定性复扫这一项。

**写后同轮清零**：正文落盘不是汇报时机——每章落盘后必须在**同一轮**内跑完上方步骤 10-11b 扫描、下方确定性收尾脚本与 narrative-writer 审查，blocking 清零才算本章完成；不得先汇报"已写完"再等指示。写后 hook 会对落盘正文自动扫描确定性毒句式并把命中推回——那是兜底网不是替代，hook 报出的命中当轮清零。**唯一豁免**：用户显式说"本章不去味/跳过检查"——豁免时在该章标题行下加一行 `<!-- 去味:跳过 -->`（写后 hook 的毒句式推回与写下一章前的欠账拦截都认这个标记；其余网照常）。**注意：豁免"去味"不等于豁免"文风对齐"**——步骤 8b 的指标门与 10b 的作者基线比对仍须执行（它们防的是文风漂移，不是 AI 味）。

**确定性收尾**：本批正文写完后，主会话对实际落盘文件依次运行：
1. `node scripts/check-ai-patterns.js --check --fail-on=blocking 正文/第XXX章_*.md` — AI 味组合句式
2. `node scripts/check-outline-copy.js 正文/第XXX章_*.md` — 细纲照搬复扫
3. `node scripts/style-metrics.js --profile {文风路径} 正文/第XXX章_*.md` — **文风偏离门**（步骤 8b）
4. `node scripts/normalize-punctuation.js --quote-mode {文风声明的体系} 正文/第XXX章_*.md` — 标点收尾

blocking 命中先回正文改写并复扫；advisory 与细纲重合逐条读原文判断，确属问题才改，功能性写法标 `[需复核]`——每条都要有结论，不为归零机械改写；细纲重合里判定保留的补进细纲「复沓锚句」，下章起不再复报。其中 `formulaic-parallelism` 必须连同对话一起复核，不能因为 hook 不阻断台词就略过。

> **`normalize-punctuation.js` 的两个开关都必须按文风声明取值**：
> 1. **`--quote-mode`** 取 `文风.md` 声明的引号体系（未声明才用默认 `curly`；声明角引号传 `corner`，声明半角双引号传 `ascii`）。用默认值硬刷会把作者的引号体系刷成另一种——**实测踩过：原文弯引号、续写被写成角引号，同一本书三章之内出现两套体系**。先用 `--check` 报出混用，再决定刷哪一边。
> 2. **`--keep-ellipsis`**：当 `文风.md` 的 `ellipsis_per_kilo` **基线非零**时**必须加此开关**。该脚本默认把 `……` 当无功能标点清掉（并报 `ellipsis` finding），而作者本人用省略号时它是语气武器——实测某书作者 `……` 出现 1582 次、密度 1.28/千字，误清即抹掉声纹。
> **破折号 `——` 与双连字符 `--` 仍照改**，无作者基线例外（见 `banned-words.md`）。
