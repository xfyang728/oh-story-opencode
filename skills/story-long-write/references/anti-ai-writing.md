...
### Show Don't Tell 的反向失衡（v2.1 新增，实测高发）

上面四条被过度执行时，会产生**比 AI 味更难发现的漂移**：**把连续镜头压成旁白概括**。
症状是每一句都"有信息"，但没有一处让读者看见——文本变成提纲。

| 反向失衡 | 错例（实测） | 修法 |
|----------|--------------|------|
| 用概叙跳层代替场景 | "第三层。／第五层。／第八层，幻境开始成阵……／第十层。／如履平地。" | 关键情节拍成场景；跳层只能用于非关键过渡，**整章不超过一次** |
| 用抽象总括代替反应链 | "所有家族子弟，都停下了动作。"／"全场死一般的寂静。" | 拆成具体人的具体失态（谁的笔掉了、谁把话咽回去、谁的手在抖） |
| 用叙述者解释代替角色算账 | 让主角坐着听人讲"十个队伍、满编一百二十人、一百个名额被占死" | 让利益相关方在**下命令或算账**时把数字吐出来（情报官数名额、执事算席位） |
| 用总结句代替实物镜头 | "他的全部家当，都在内景地和面板里。" | 给实物：六只白瓷瓶、压进箱底的木匣、留作下次引信的十六枚丹药 |
| **主语延续式长段**（v3 新增，最高发） | 一段 108 字塞 6 个句号级单句："他今天不是来听这个的。飞舟明日清晨靠港，他只剩今天。九州的坊市他走过十年。十年前他拉着车从这条街口过……" | **按句拆段**——不是删句！作者写同样的"主语延续"时**每句独立成段**：「一阶炼丹师，便能炼制九品、八品、七品的丹药。」／「他们的身份，高贵到了极点。」／「因为学习炼丹术的门槛，太高了。」 |
| 一段多拍 | 一个段落里完成"动作＋感知＋判断＋过渡"四个镜头 | 一段一拍。段界就是节奏，拆段优先于删词 |

**判据**：连续三个以上"总括式短句"而不见具体动作/实物/对话 → 判为概叙，回写。
可用 `style-metrics.js` 的 `narrator_explain_ratio`（概叙比，只卡上限）与**段落长尾四项**
（`ge60_para_ratio` / `p90_para_len` / `max_para_len` / `multi_beat_para_ratio`）做机械兜底。

### 段落长尾：`Show Don't Tell` 之外的另一条命门（v3 新增）

**平均段长骗人。** 实测同一本书，作者与 AI 续写的平均段长只差 1.7 字，长尾差 4 倍：

| 段落指标 | 作者 461-466 | AI 续写 464-466 |
|---|---|---|
| 平均段长 | 20.67 字 | 22.41 字 |
| 段长 P90 | 40 字 | 47 字 |
| 最长段 | **73 字** | **108 字** |
| 段长≥60 字占比 | **2.2%** | **6.3%** |
| 单段≥4 拍占比（叙述段） | 0.4% | 1.5% |

读者的体感"读起来累"来自那 4% 的长尾，而平均值把它抹平了。两条**不依赖基线**的绝对硬线：

- **单段 ≥80 字 → blocking**（手机上超过 4 行，必须拆）
- **单段 ≥60 字且 ≥5 拍 → blocking**（主语延续式长段）

修法永远是**拆段**，不是删句。删句会把作者的语气和节奏一起删掉——正如 `banned-words.md`
的"删多余修饰"受作者基线约束，"压长短段"同样受段落基线约束。

> **允许的例外：说明性长块**。作者交代新规则时确实会连写多句（如"封印真意／只能用窍穴发力／
> 隐藏身份"，每句一段）。**这不是负空间违反**——见 `style-set-piece-templates.md` 的模板 B/T2：
> 允许连续单句成段的说明块，但**每段只放一个信息点**。判据是"每段几个信息点"，不是"总长度"。

**反应层配额（打脸/揭示/装逼章专用）**：

- 主角内心独白与回忆**合计不超过全章 15%**，且**不得占据揭示后的第一顺位**。
- 揭示/打脸发生后的第一顺位**必须给旁观者**：先写别人的失态，再写主角的反应。
- 在场配角的反应链按「辱骂 → 见牌/见数呆滞 → 失态 → 退场或服软」写足，
  **反应段落篇幅 ≥ 主角段落 × 3**（参考作者原文锚点：陈渊听到结果的四段式，或巡捕见牌的
  辱骂→呆滞→自扇嘴巴→点头哈腰）。
- 爽感长在**别人脸上**，不长在主角心里。若本章最用力的是主角的心理活动，判不达标。

**细节位配额（配合 writing-craft.md「细节是推进器」）**：

- **章首／每个转场后的第一句，必须是可换算的细节**——威胁评估、资源盘点、判断依据；**禁止氛围描写占位**。
  自检：删掉这句，读者对**当前处境**的理解会不会变差？不会变差 → 抒情占位，回写。
- **抒情／回忆／内心独白合计不超过全章 15%**，且**不得占据章首与揭示后的第一顺位**。
- **数字分两轨**：算账数字（能推导结论）与标尺数字（能判断强弱）→ 保留写足；
  背景数字（人口／年份／规模，不参与任何决策）→ **删，或改为可判断的形式**。
- **设定说明书禁令**：任何连续 3 行以上的背景交代，若在场无人据此做决定，一律拆成
  "算账／下命令／做选择"的碎片。**把背景换到反派嘴里讲，不算修法**。
...
### 3. 格式一致性
- 对话格式统一：对话与引用一律使用半角双引号 `""`，禁止「」/`『』`（不再保留盐言例外）
...
> **作者基线优先（v2.1）**：本模式与「模式 2 弱化副词泛滥」在仿写任务里会与 `文风.md` 的
> 「作者指纹词（正向白名单）」直接冲突。**冲突时以白名单为准**：命中的词落在频次带内就保留，
> 不要以"弱化副词"为由清零——作者的母语级口癖不是 AI 指纹。判级与例外口径见
> `banned-words.md` 的「判定优先级」。同理，作者基线非零的 `……`/`——` 不得按"无功能标点"清空。
