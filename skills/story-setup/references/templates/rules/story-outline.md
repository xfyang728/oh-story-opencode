...
2. **细纲必填项**：每个细纲（细纲_*.md）必须包含当前“章节蓝图”：
   ...
   - 情节细化：情节点序列写成表格 `# | 情节点（谁做了什么） | 功能标签 | 密/铺/疏 | 目标字数`，目标字数写区间（**档位以 story-long-write 技能 Phase 3「情节细化」的档位规则为准**，含 `<1500` 字短章的密点下浮例外；不在本文件复制具体数字），末尾写一行 `目标字数合计：下限X字（章目标Y，范围Y-Z）`，并标 行动成本（可无）/收益归属——行动成本可无，不硬造代价；旧版细纲只有 代价兑现 / 收益兑现 时回退按该字段核对，不阻塞
