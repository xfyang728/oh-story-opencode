...
- **spawn 报 Model not found 时的兜底**：部署后若某 agent spawn 报 Model not found（如 opencode 托管模型不含该 ID），删除该 agent 的 `model:` 行继承会话默认，或改用该环境实测可用 ID（本项目实测：opencode/x-preview-f-free）
