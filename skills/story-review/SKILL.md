...
4. **检查核心 Agent 部署状态**（检查项目内 agents，同时兼容 Claude Code、OpenCode 和 Codex）：
   - 优先检查 `.opencode/agents/`，其次检查 `.claude/agents/`，再检查 `.codex/agents/`；三个目录任一存在即视为已部署
    ...
    - 对每个必需 Agent 文件：
      - **OpenCode / Claude Code agent（`.opencode/agents/`、`.claude/agents/`）**：读取 frontmatter，确认 `name:` 与 subagent_type 完全一致；frontmatter 缺失、不可解析或 name 不匹配时视为 malformed agent。
...
1. `{项目根}/skills/{规范路径}`（项目内安装；Claude Code 项目为 `{项目根}/.claude/skills/{规范路径}`）
...
6. **确定性预检（只报告，不修改）**：当审查范围包含本地正文文件路径时，运行本 skill 自带脚本：
   ...
   - 引号检查：对话与引用一律应为半角双引号 `""`；发现 `「」`/`『』` 即给出转换建议（不再保留盐言例外）。

**story-explorer 预查询（可选）**。仅当 `Effective Mode` 仍为 `full`/`lean`、当前允许 spawn 且 Agent/Task 工具可用时，才可检查 agent 目录（优先 `.opencode/agents/`，其次 `.claude/agents/`，再检查 `.codex/agents/`）下的 `story-explorer.md` 或 `story-explorer.toml` 并 spawn `story-explorer` 预查设定摘要；`solo` 或子代理递归保护场景下不得 spawn，只能直接 Read/Grep。Prompt 示例：
