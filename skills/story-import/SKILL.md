...
- 只有 `agents_version: 25` 通过后，才按 `.opencode/agents/chapter-extractor.md` → `.claude/agents/chapter-extractor.md` → `.codex/agents/chapter-extractor.toml` 检查 Phase 2 长篇并行 agent。
...
- 可选验证：如果项目已部署 story-explorer agent（优先检查 `.opencode/agents/` 下的 `story-explorer.md` 是否存在；不存在时再检查 `.claude/agents/`，再不存在时检查 `.codex/agents/`），可使用 Task 工具 spawn story-explorer 子代理（subagent_type: "story-explorer", prompt: "项目目录：{dir}\n查询类型：progress\n查询参数：导入验证"）交叉验证迁移数据完整性
