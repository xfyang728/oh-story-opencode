...
钩子类型的断章句法细分，供 Stage 1 标注「章尾钩子」与写作端选用：

| 子类 | 机制 | 示例 |
|------|------|------|
| 机制悬念 | 金手指/核心机制的运作方式留而未揭 | 「面板到底怎么结算？」 |
| 新目标 | 章尾确立下一阶段具体目标 | 搞钱/搞药/搞军功 |
| 身份危机 | 马甲/底牌险些暴露 | 差一步被认出真实身份 |
| 三字诀断章 | 三五字短语独立成段收章 | 「神通子！！」 |
| 问号断章 | 疑问句收章制造期待 | 「这道神通名为：佛怒火莲？」 |
| 能力名衔接下章标题 | 章尾能力名＝下章标题，无缝追读 | 章尾「佛怒火莲」→ 下章《佛怒火莲》 |

> 前三型（机制悬念/新目标/身份危机）建议轮换使用，防单一钩型疲劳；后三型为断章句法形态，可与前三型叠加。
...
| 段落 | 内容 | 必标可信度 | 上限 |
|---|---|---|---|
| 生成记录 | 参考了哪些资料 + 抽样看了哪几章 + 文风是否可用 + **判定窗口 + 基线更新章号** | 否 | <150 字 |
| 整体语感 | 句长分布 / 标点习惯 / 段落节奏 / **缩进与段间** / **引号体系** | 是（每子项一标） | ~600 字 |
| **写作指标带** | 双层基线（**全书兜底 + 滚动基线（近9章）**）：对话占比 / 叹号·问号·省略号·破折号密度 / 场次切换数 / 面板密度 / 平均段长 / **段长中位数 + P90 + 最长段** / **段长≥60字占比** / **单段≥4拍占比** / 概叙比——**数值由 `style_stats.py metrics` 实测**，附 `metrics: available\|unavailable` | 是（整体一标） | ~600 字 |
| **作者指纹词** | 8-15 个单字级高频口癖词；每词给 `全书频次带` + **`近9章实测`** + **判定带（0.6×~1.6×）** + **门级（近9章 ≥3.0/万字 才 blocking）**；另设 advisory 层记「近期腔调词」 | 是 | ~350 字 |
| **负空间** | 作者不写什么（4-6 条） | 是 | ~150 字 |
| **硬禁高危组合** | 从 banned-words 组合级里挑 5-10 条本书最易复发项，标注不可被白名单豁免 | 是 | ~150 字 |
| **定式场景模板** | 4-6 个本书高频定式（入新场／开新地图与新规则／队伍汇合／强者立威／人情交换）的**节拍序列 + 篇幅配比 + 允许的说明段形态 + 原文锚点行号** | 是 | ~500 字 |
...
> **v3 新增必填项**：`滚动基线` 列、`判定窗口 + 基线更新章号`、段落长尾四项、
> 指纹词的 `近9章实测 + 判定带 + 门级`、`定式场景模板` 段。
> 它们分别解决四类已实测的漂移：**作者文风滚动**（静态基线误报作者本人）、
> **段落长尾**（平均段长看不见 4 倍差距）、**短窗口词频方差**（把作者判成"把词压掉了"）、
> **定式场景不成形**（指标全合格但仍不像本书）。
> 生成方法见 [style-profile-generator.md](style-profile-generator.md) Step 4 / 4b / 4c。
