...
#### 源文本异常协议（Stage 0.5 产出章节边界表时同步执行）

| 异常类型 | 判定规则 | 处置 |
|----------|---------|------|
| 缺章标题 | 章号编号跳变且原文无对应标题（如 121→123，缺第122章） | 章节边界表按 `第{N}章_缺失` 占位登记；不虚构该章摘要 |
| 重复章标题 | 同一「第N章」标题出现 ≥2 次 | 按 `第{N}章_重a`、`第{N}章_重b` 后缀区分为独立章，各自切片提取 |
| 空占位章 | 单章字数 < 100 | 标记占位章，不 spawn 提取 agent，摘要文件写「空占位章，无有效内容」 |

- 命名约定：异常章一律用 `_` 后缀标记（如 `第122章_缺失`），保证章节边界表章号列可排序、下游切片不受影响
- `_progress.md` 失败记录格式：`| 源文本异常 | 第122章 | 缺章标题（121→123 跳变） | 已按协议处置 |`——每处异常一行，逐条登记
...
#### C. 小说概要生成（在 Stage 5 落盘，覆盖 Stage 0 thin first-pass）
...
| 体量 | 协议 |
|------|------|
| <50 章 | 按「开局卷精拆」处理：Stage 3 允许覆盖率 100% 特例（豁免 95% 上限；条件：单主线时序切分 + 复核无强行并条）；剧情条粒度放宽至 4-6 章/条 |
| ≥200 章 | 启用跨块合并启发式（见下方「跨块合并」节的剧情合并启发式） |

> 与「质量阈值体系」节小体量节选特例（<30 章）的关系：本协议是其体量上限的扩展（<50 章），复核重点同为「有无把不同剧情强行并进同一条」，而非覆盖率数值本身。
...
**剧情合并启发式**（Stage 3 二次合并的显式规则）：
1. **因果闭环才合并**：两条剧情合成一个完整因果闭环（目标—冲突—解决）才可合并；仅前条结尾引出后条开头的弱衔接不算
2. **对手同链可合并**：同一对手链上的连续对抗（小弟→头目→幕后主使）可并为一条
3. **跨扫描块边界默认不合并**：分属不同扫描块的剧情默认保持独立，除非同时满足上方四项合并判断标准
4. **合并后单弧 ≤15 章**：合并产生的单条剧情超过 15 章即过粗，禁止合并、保留拆分
